...
namespace LogicAppTriggers.Controllers
{
    public class WebhookTriggerController : ApiController
    {
        ...
        public HttpResponseMessage Subscribe(string callbackUrl)
        {
            subscriptions.Add(callbackUrl);
            new Thread(() => doWork(callbackUrl)).Start();   //Start the thread of work, but continue on before it completes
        ...
        public HttpResponseMessage Unsubscribe(string callbackUrl)
        {
